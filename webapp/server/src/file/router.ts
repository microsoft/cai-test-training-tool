...
var uploadFileToBlob = async (file) => {
  if (!file) return [];
...
var createBlobInContainer = async (containerClient, file) => {
  // create blobClient for container
  var blockBlobClient = containerClient.getBlockBlobClient(file.name);
  var res = await blockBlobClient.upload(file.data, file.size);
};
...
router.post("/", function (req, res) {
  if(!req.user.permissions.includes("BMT_QNA_Tester")){
    ...
    if (!req.files || Object.keys(req.files).length === 0) {
      ...
      uploadFileToBlob(req.files.files)
        .then((result) => {
